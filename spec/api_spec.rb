...
describe "API" do
  ...
  describe 'POST /send' do
    ...
    it 'should respond with status 400 if params token, sender, recipient, subject or body are missing from the submission' do
      post '/send'
      last_response.status.must_equal 400, 'Expected status 400'
      last_response.body.must_match /Params token, sender, recipient, subject and body are required/
    end

    it 'should respond with status 403 if params token is invalid' do
      post '/send', params={token: 'bad_token',
                            sender: 'john@domain.com',
                            recipient: 'jane@domain.com',
                            subject: 'Test',
                            body: "Test"}
    ...
    it 'should process a submission containing valid params' do
      ...
      last_result['recipient'].must_match /jane@domain.com/
